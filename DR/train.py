...
def train(args, model):
    ...
    train_dataset = MSMARCODataset("train", args.msmarco_dir, 
            ...
            args.max_query_length, args.max_doc_length, args.insert_typo)
...
def run_parse_args():
    ...
    parser.add_argument("--tokenize_dir", type=str, default="./data/msmarco-passage/queries.train.tsv")
    ...
    parser.add_argument("--insert_typo", default=0, type=int)

    args = parser.parse_args()

    time_stamp = time.strftime("%b-%d_%H:%M:%S", time.localtime())

    if args.insert_typo == 1:
        args.output_dir = args.output_dir + "_typo"
...
def main():
    ...
    config = BertConfig.from_pretrained(load_model_path, cache_dir=".cache")
    model = RepBERT_Train.from_pretrained(load_model_path, config=config, cache_dir=".cache")
