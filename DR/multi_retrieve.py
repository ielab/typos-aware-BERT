...
def writer(args, finish_queue_lst):
    ...
    with open(args.output_path, 'w') as outFile:
        for qid in tqdm(query_id_memmap, desc=f"writing {args.query_embedding_dir}"):
            score_docid_lst = []
...
@raise_immediately
def allrank(gpu_queue, doc_begin_index, doc_end_index, finish_queue, args):
    import os
    import torch
    gpuid = gpu_queue.get()
...
if __name__ == "__main__":
    ...
    print(args.query_embedding_dir)
    ...
    for finish_queue, (doc_begin_index, doc_end_index) in zip(finished_queue_lst, doc_arguments):
        pool.apply_async(allrank, 
            args=(gpu_queue, doc_begin_index, doc_end_index, finish_queue, args))
